...
class generator;
  ...
  task run();
    ...
    repeat(count) begin
      ...
      mbx.put(tr);  //send to driver class
      tr.display("Gen");
      @(nextdrv); //wait till driver completes its task
      @(nextsco); //wait till scoreboard completes its task
    end
    -> done; //after completing generation of count number of stimuli
  endtask
endclass
...
class driver;
  ...
  virtual apb_if vif;
  mailbox #(transaction) mbx;
  transaction datac; //to store data rcvd from gen class
  event nextdrv;
