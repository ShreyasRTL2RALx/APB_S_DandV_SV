module apb_s(
  input pclk,
  input presetn,
  input [31:0]paddr, //32 bit address to understand operation of slverror
  input psel,
  input penable,
  input [7:0] pwdata,
  input pwrite,
  ...
  reg [7:0] mem[16]; //memory capable of storing 8 bits
  
  reg [1:0] state, nstate;//state variables
  
  bit  addr_err , addv_err, data_err;
  ////////// setup - correct apb cycles
  /////////  addr_range - should be less than 16 else trigger addr_err
  /////////   addr_val - must be 0 or 1, presence of X or Z triggers addv_err
  /////////   data_val -must be 0 or 1, presence of X or Z triggers data_err
  ///if any one err is triggered, slverr will be high
...
*/
  ...
  //reset decoder
  always @(posedge pclk, negedge presetn) begin //asynchronous reset
    if (presetn == 1'b0)
      state <= idle;
    else 
      state <= nstate;
  ...
  always @(*)
    begin 
      case (state)
        idle:
          begin 
            prdata = 8'h00; //set to default in idle state 
            pready = 1'b0;
  ...
  //detection address value error 
  reg av_t=0;
  always @(*)
    begin
      if (paddr >= 0)
        av_t = 1'b0;
      else               ///presence of X or Z
        av_t = 1'b1;
    end
